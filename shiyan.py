...
def train_full(yaml: dict):
    ...
    total_lab, total_train_acc, total_paras = 0.0, 0.0, 0.0
    ...
    for epoch in range(yaml['para']['epoch']):
        for x, y, para in la_dataloader:
            ...
            total_loss = loss + loss_para * 0.00001
            train_acc = torch.mean((pc.max(1)[1] == y).float())
            total_loss.backward()
            optimizer_D.step()
            total_lab += loss
            total_paras += loss_para
        ...
        with torch.no_grad():
            for test_x, test_y, test_para in test_dataloader:
                ...
                loss_paras_test = F.l1_loss(test_para, test_pp, reduction="none").sum()
                test_acc_top1 += acc1
                test_acc_top2 += acc2
                test_acc_top3 += acc3
            ...
            logging.info(
                f'Epoch:{epoch + 1}: test_acc_top1:%.4f test_acc_top2:%.4f test_acc_top3:%.4f test_para_loss:%.2f best_top1:%.4f best_top2:%.4f best_top3:%.4f best_para:%.2f' % (
                    test_acc_top1, test_acc_top2, test_acc_top3, test_paras_loss, best_acc1, best_acc2, best_acc3, best_para / 100))
    logging.info(f'Done!\nTesting full-supervised model performance...')
    # return test_net(yaml, D)


def train_cls_finetine(yaml: dict):
    label_path = yaml['para']['label_train_path']
    label_xml_path = yaml['para']['label_xml_path']
    label_test_path = yaml['para']['label_test_path']
    unlabel_path = yaml['para']['unlabel_path']
    batch_size = yaml['para']['batch_size']
    lr = 0.0001
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])
    ...
    for epoch in range(yaml['para']['epoch']):
        ...
        for x, y, _ in lab_dataloader:
            try:
                ...
                total_train_acc /= len(ulab_dataloader)
                total_unlab /= len(ulab_dataloader)
                total_gen /= len(ulab_dataloader)
                total_paras /= len(ulab_dataloader)
                total_lab /= len(ulab_dataloader)
                ul_iter = iter(ulab_dataloader)
                ul2_iter = iter(ul2_dataloader)
                ul1 = next(ul_iter)
                ul2 = next(ul2_iter)
                logging.info(f'Epoch:{epoch}: loss_lab:%.4f loss_ulab:%.4f loss_gen:%.4f train_acc:%.4f loss_para:%.2f' % (
                    total_lab, total_unlab, total_gen, total_train_acc, total_paras / batch_size))
            ...
            lab = D(x, feature=True)
            unlab = D(ul1, feature=True)
            gen = G(noise1)
            ...
            optimizer_D.step()
            # cls-finetuning
        total_lab /= len(lab_dataloader)
        total_unlab /= len(lab_dataloader)
